...
float posX =0, posY = 2.5, posZ =-3.5, rotRodIzq = 0, rotRodDer = 0, giroBrazoIzq=0, giroBrazoIzqInc=0, giroBrazoDer = 0, giroBrazoDerInc = 0;
float giroMonito = 0, rotCin = 0, rotCinInc =0;

#define MAX_FRAMES 60	//Maximo 5 keyframes
int i_max_steps = 90;		///		 Velocidad a la que corre la animación
int i_curr_steps = 0;
...
void saveFrame ( void )				// Guardar las posiciones que tiene el personaje
{
	...
	KeyFrame[FrameIndex].posX=posX;		// Variables del dibujo del personaje
	...
	KeyFrame[FrameIndex].rotCin = rotCin;
...
void interpolation ( void )
{
	...
	KeyFrame[playIndex].rotCinInc = (KeyFrame[playIndex + 1].rotCin - KeyFrame[playIndex].rotCin) / i_max_steps;
}
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				// agregado
				glRotatef(25, 0, 0, 1);
				glRotatef(25 + giroBrazoDer, 1, 0, 0);
		...
		glPushMatrix();//Cintura
			...
			glPushMatrix(); //Pie Derecho -->
				...
				glPushMatrix();
					glTranslatef(0, -0.5, 0);
					/// SE AGREGO RodDer para rotar la rodilla DER
					glRotatef(15+rotRodDer, 1, 0, 0);
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	textPata.LoadBMP("casa/brickwall.bmp");
	...
	textCuadro1.LoadBMP("casa/sky2.bmp");
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			rotCin += KeyFrame[playIndex].rotCinInc;
